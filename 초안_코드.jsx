  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      <div className="toolbar">
        <Btn v="primary" s="sm" onClick={()=>onNav("S09")}>+ 시드 추가</Btn>
        <Btn v="secondary" s="sm" onClick={()=>onNav("S10")}>변경 이력</Btn>
        <Btn v="secondary" s="sm" onClick={handlePrune}>가지치기 추천</Btn>
        <Btn v="secondary" s="sm" onClick={()=>onNav("S29")}>내보내기</Btn>
        <div style={{flex:1}} />
        <div className="search-wrap" style={{width:220}}>
          <span>🔍</span><input placeholder="노드 검색..." value={q} onChange={e=>setQ(e.target.value)} />
        </div>
      </div>
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        {/* Canvas */}
        <div style={{flex:1,padding:20,overflow:"hidden",position:"relative"}}>
          <div style={{height:"100%",background:TDS.bgTertiary,borderRadius:16,position:"relative",border:`1px solid ${TDS.borderDefault}`}}>
            {loading && (
            ...
            {!loading && !nodes.length && (
              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:14,color:TDS.textTertiary}}>
                <div style={{fontSize:40,opacity:.4}}><TFI s={40} color={TDS.textTertiary}>🕸️</TFI></div>
                <div style={{fontSize:14}}>아직 노드가 없습니다</div>
                <Btn v="primary" s="sm" onClick={()=>onNav("S09")}>+ 첫 시드 추가하기</Btn>
              </div>
            )}
            {/* 엣지 (store 기반) */}
            <svg style={{position:"absolute",inset:0,width:"100%",height:"100%"}}>
              {edges.map(e=>{
                ...
                if(!a||!b) return null;
                return <line key={e.id} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={TDS.borderStrong} strokeWidth="1.5" strokeDasharray="4 3" opacity=".7" />;
              })}
            </svg>
            {/* 노드 (store 기반) */}
            {nodes.map(n=>{
              const dim = matched && !matched.includes(n.id);
              return (
              <div key={n.id} onClick={()=>setSel(sel?.id===n.id?null:n)} style={{position:"absolute",left:n.x,top:n.y,transform:"translate(-50%,-50%)",width:n.size,height:n.size,borderRadius:"50%",background:n.color,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:n.size>50?11:9,fontWeight:700,cursor:"pointer",transition:"transform .18s, opacity .18s",opacity:dim?.25:1,boxShadow:sel?.id===n.id?`0 0 0 3px #fff, 0 0 0 5px ${n.color}`:"0 3px 10px rgba(0,0,0,.18)"}}>
                {n.label}
              </div>
              );
            })}
            <div style={{position:"absolute",bottom:16,right:16,display:"flex",gap:8}}>
              {["−","+"].map(z=><button key={z} className="btn-inline" style={{width:32,height:32,padding:0,display:"flex",alignItems:"center",justifyContent:"center"}}>{z}</button>)}
            </div>
          </div>
        </div>
        {/* Detail panel */}
        {sel&&(()=>{ const eList=edgesOf(sel.id); return (
          <div style={{width:360,background:TDS.bgPrimary,borderLeft:`1px solid ${TDS.borderDefault}`,padding:24,overflowY:"auto"}}>
            <div className="row-between mb16" style={{marginBottom:16}}>
              ...
              <button onClick={()=>setSel(null)} style={{background:TDS.bgTertiary,border:"none",width:28,height:28,borderRadius:8,cursor:"pointer"}}>✕</button>
            </div>
            <div style={{fontSize:22,fontWeight:800,color:TDS.textPrimary,marginBottom:8}}>{sel.label}</div>
            ...
            {eList.length ? eList.map((e,i)=>{
              ...
              return (
              <div key={e.id} style={{padding:"10px 12px",background:TDS.bgTertiary,borderRadius:10,marginBottom:8}}>
...
function S25() {
  const notifs=[
    {ic:"💬",t:"새 코멘트",d:"김선생님이 양자컴퓨팅 노드에 코멘트를 남겼습니다.",time:"10분 전",read:false},
    {ic:"🧠",t:"PDF 파싱 완료",d:"생기부 PDF 분석이 완료되었습니다. 결과를 확인하세요.",time:"1시간 전",read:false},
    {ic:"✂️",t:"가지치기 추천",d:"8개의 새로운 노드 가지치기 추천이 있습니다.",time:"3시간 전",read:true},
    {ic:"📄",t:"양식 생성 완료",d:"화학과 지원용 자소서가 생성되었습니다.",time:"어제",read:true},
    {ic:"🔔",t:"시스템 공지",d:"2026년 2학기 생기부 제출 마감일 안내",time:"3일 전",read:true},
  ];
  return (
    <div className="content" style={{maxWidth:640,margin:"0 auto"}}>
      <div className="row-between mb24" style={{marginBottom:24}}>
        <div className="sec-title">알림</div>
        <Btn v="secondary" s="sm">모두 읽음 처리</Btn>
      </div>
      {notifs.map((n,i)=>(
        <div key={i} style={{padding:"16px 20px",borderRadius:14,marginBottom:10,background:n.read?TDS.bgPrimary:TDS.blue50,border:`1px solid ${n.read?TDS.borderDefault:TDS.blue100}`,display:"flex",gap:14,cursor:"pointer"}}>
          <div style={{fontSize:24,flexShrink:0}}><TFI s={24} color={TDS.textSecondary}>{n.ic}</TFI></div>
          <div style={{flex:1}}>
            <div style={{fontSize:15,fontWeight:n.read?500:700,color:TDS.textPrimary}}>{n.t}</div>
            <div style={{fontSize:13,color:TDS.textSecondary,marginTop:3}}>{n.d}</div>
          </div>
          <div style={{fontSize:12,color:TDS.textTertiary,flexShrink:0}}>{n.time}</div>
        </div>
      ))}
    </div>
  );
...
/* ──────────────────────────────────────────────────────────────
   LAYOUT HELPER
────────────────────────────────────────────────────────────── */
function getLayout(s){ if(["S01","S03","S04","A00"].includes(s))return "full"; if(s.startsWith("T"))return "teacher"; if(s.startsWith("A"))return "admin"; return "student"; }
...
/* 화면 id → 요구 역할 (게이팅) */
function requiredRole(s){ if(s==="A00"||s.startsWith("A"))return "admin"; if(s.startsWith("T"))return "teacher"; return "student"; }
...
function AppShell() {
  ...
  useEffect(()=>{
    ...
    if (session) {
      const need = requiredRole(screen);
