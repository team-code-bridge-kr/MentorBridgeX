...
**왜 미래 날짜를 버리는가** — 원문 쪽 날짜가 잘못 오는 경우가 있다. 일부 학술지가
DOAJ 를 거쳐 **2109~2121년**으로 들어왔다(이슬람력을 잘못 환산한 값으로 보인다).
최신순 정렬에서 그런 글은 영원히 맨 앞을 차지한다 — 실제로 최신 40건이 전부 그
논문이라 대시보드 "오늘의 관심 기사"에 뉴스가 한 건도 뜨지 않았다. 두 겹으로 막는다:

- **수집**(`ingest/runner.py`): 이틀 넘게 미래면 날짜를 모르는 것으로 보고 수집
  시각으로 채운다. 글 자체를 버리지는 않는다.
- **조회**(`feed.py`): `published_at <= now + 2일` 로 한 번 더 거른다. 이미 들어와
  있는 잘못된 행(143건)이 피드를 점유하지 못하게 하는 안전망이다. 저장함 탭은
  사용자가 직접 담은 글이라 거르지 않는다.

기존 143건은 **날짜를 고치지 않고 감춘다.** 진짜 발행일을 알 수 없는데 수집
시각으로 바꾸면, 2021년 논문이 "오늘 올라온 글"로 맨 앞에 서서 증상만 옮겨 간다.
