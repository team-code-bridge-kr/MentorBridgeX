...
const RETIRED = {
  ...
  /* 통계 → 대시보드. 화면의 절반이 틀린 값을 그리고 있었다 — 「영역별 완성도」는
     생기부를 다 채운 사람에게도 전부 0% 였고(공백 문자열의 길이를 재고 있었다),
     「노드 분포」는 모든 막대의 값을 1 로 박아 두어 200개 노드가 죄다 같은
     길이로 섰다. 나머지 숫자(노드·엣지·양식 수)는 대시보드와 그래프가 이미
     제자리에서 말한다. */
  S28: "S05",
  /* 프로필 편집 → 설정. 설정 화면과 똑같은 넉 줄(이름·이메일·역할·제공자)을
     다시 보여주면서 이름 한 칸만 고칠 수 있었고, 나머지 절반은 노드·엣지
     개수라 그래프가 제자리에서 말한다. 이름은 이제 설정에서 바로 고친다. */
  S31: "S30",
};
