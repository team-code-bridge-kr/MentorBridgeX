...
/* 자식 고리 — 동그라미가 아니라 **타원**이다.
 *
 * 캔버스는 가로로 길다(대개 3:2 안팎). 자리를 백분율로 주면서 x·y 에 같은
 * 반지름을 쓰면, 같은 27% 가 세로로는 훨씬 짧은 거리라 위아래 이웃이 좌우
 * 이웃보다 가깝게 붙는다. 화면 모양을 따라 가로를 넓게 잡는다.
 *
 * 바깥 고리는 50+rx 가 96(pct 의 상한)을 넘지 않아야 한다. 넘으면 잘려서
 * 여러 자리가 같은 끝점에 겹쳐 앉는다.
 *
 * 고리는 **둘**이다. 셋으로 나누면 가로 상한(96) 때문에 바깥 둘의 rx 가 거의
 * 같아지고, 가로축(cos=±1)에서 두 고리의 자리가 같은 점에 앉는다. 그 자리를
 * 나눠 갖느니 한 고리에 넉넉히 두르는 편이 낫다 — 층마다 한 화면을 쓰게 된
 * 뒤로 자식이 스물을 넘는 층 자체가 드물다. */
const RINGS = [
  { rx: 38, ry: 30, cap: 8 },
  { rx: 45, ry: 41, cap: 18 },
];
 ...
 * 자식들을 고리에 나눠 담는다.
 *
 * 한 고리에 다 밀어 넣으면 서로 붙는다. 고리마다 반 칸씩 어긋나게 돌려서 안팎이
 * 한 줄로 서지 않게 한다 — 나란히 서면 어느 것이 어느 고리인지 눈으로 못 가른다.
 *
 * 한 고리에 담는 수를 10 → 8 로 줄이고 반지름을 키웠다. 층마다 한 화면씩
 * 쓰게 된 뒤로 한 번에 뜨는 노드가 확 줄었으니, 아껴 쓰던 자리를 이제 넓게
 * 쓴다. 노드는 지름이 60px 안팎이고 이름표가 그 아래로 더 나오므로, 자리
 * 사이가 노드 하나 폭보다 넓어야 이름이 서로를 가리지 않는다.
 */
export function ringSlots(count) {
  ...
  rings.forEach((ring, ri) => {
    ...
    for (let i = 0; i < ring.n; i += 1) {
      ...
      out.push({
        x: CENTER.x + ring.rx * Math.cos(a),
        y: CENTER.y + ring.ry * Math.sin(a),
      });
    }
  });
  return out;
